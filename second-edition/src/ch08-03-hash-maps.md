...
この説でマッシュマップの基礎的なAPIを見ていきますが、より多くのグッズが標準ライブラリにより、
`HashMap<K, V>`状に定義された関数に隠されています。いつものように、
...
名前がブルーとイエローの2チームのスコアを追いかけています。ブルーチーム10点から、イエローチームは50点から始まります:
...
そこには`HashMap`も含まれています。例として、チームメイト初期スコアが別々のベクタに含まれていたら、
...
コンパイラは指定しない限りどれがいるのかわからないからです。ところが、キーと値の型引数については、
...
ベクタでしたように、`for`ループでハッシュマップのキーと値のペアを走査することができます:
...
リスト8-22のコードは、`insert`を二度呼んでいるものの、ハッシュマップには一つのキーと値の組みしか含まれません。
...
`entry`の2回目の呼び出しはハッシュマップを変更しません。なぜなら、ブルーチームはすでに`10`という値があるからです。
...
例えば、リスト8-24は、各単語があるテキストに何回出現するかをかぞえあげるコードを示しています。
...
<span class="caption">単語とカウントを保持するハッシュマップを使って単語の出現数をカウントする</span>
...
母音で始まる単語にはお尻に"hay"が付け足されます("apple"は"apple-hay"になります)。
