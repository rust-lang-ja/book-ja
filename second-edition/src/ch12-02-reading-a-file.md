...
`std::io::prelude::*`はファイル入出力を含む入出力処理をするのに有用なトレイトを多く含んでいます。
...
`main`で3文を追記しました: 一つ目が、`File::open`関数を呼んでファイルへの可変なハンドルを得て、
`filename`変数の値に渡す処理です。二つ目が、`contents`という名の変数を生成して、
