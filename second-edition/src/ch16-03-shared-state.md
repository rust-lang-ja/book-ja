...
Rustの型システムと所有権ルールにより、この管理を正当に行う非常に大きな助けになります。
例を挙げれば、メモリ共有を行うより一般的な非同期処理の基礎の一つであるミューテックスを見ましょう。

<!-- ### Mutexes Allow Access to Data from One Thread at a Time -->

### ミューテックスは、一度に1つのスレッドからデータにアクセスすることを許可する
...
故に、ミューテックスをロックシステム経由で保持しているデータを*死守する*(guarding)と解説します。
...
2. ミューテックスが死守しているデータの使用が終わったら、他のスレッドがロックを獲得できるように、
   データをアンロックしなければならない。
...
結果として、ロックの解除を忘れ、ロックの解除が自動的に行われるので、
...
コンパイラがそれを正しく扱う手助けをしてくれる方法について学びます。リスト16-13が最初の例です:
...
メインスレッドでリスト16-2のように全てのjoinハンドルを収集し、それから各々に対して`join`を呼び出し、
...
この例はコンパイルできないと仄めかしたので、では、理由を探りましょう！
...
`counter`をキャプチャすることを妨げています。これは、以前では確認しづらかったことです。
...
between threads safely``と述べています。蒸留されたエラーメッセージは、`` the trait bound
`Send` is not satisfied``と述べています。`Send`については、次の節で語ります:
...
第15章で`RefCell<T>`を使用して`Rc<T>`の内部の内容を可変化できるようにしたのと同様に、
`Mutex<T>`を使用して`Arc<T>`の内部の内容を可変化しているのです。
