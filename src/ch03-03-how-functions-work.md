...
この例では、2引数の関数を生成しています。最初の引数`value`の型は`i32`、次の`unit_lael`の型は`char`です。
この関数は、`value`と`unit_label`双方の引数を含む値を出力します。
...
`x`に対して値`5`、`unit_label`に対して値`h`を渡して関数を呼び出したので、この二つの文字列は、
この値で出力されました。
